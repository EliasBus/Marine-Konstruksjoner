...
def print_elementer(elementer_utvidet):
    ...
    for elem in elementer_utvidet:
        print(f'{f'Element {int(elem[0])}-->':<17}{f'| Koord:  ({round(elem[3]/1000,2)}, {round(elem[4]/1000,2)})':<23}{f'---({round(elem[5]/1000,2)}, {round(elem[6]/1000,2)})':<20}{f'L: {round(elem[7]/1000,1)}m,':<15}{f'E: {int(elem[8]/1000)} GPa,':<15}{f'Fy: {int(elem[8]/1000)} MPa':<15}{f'Vinkel: {round(elem[14],3)}*':<18}{'|'}')
...
def print_r(r):
    print('\n\n\n---------------------------------DEFORMASJONER--------------------------------')
    for i in range(0,len(r),3):
        print(f'{f'Knutepunkt {int(i/3+1)}-->':<17}{f'| x: {int(r[i])} mm,':<20}{f'y: {int(r[i+1])} mm,':<20}{f'theta: {round(r[i+2]*180/np.pi,3)}*':<30}')
    print('---------------------------------DEFORMASJONER--------------------------------')
...
def print_utnyttelse(elementer, utnyttelse):
    ...
    for i in range(len(elementer)):
        print(f'{f'Element {int(elementer[i][0])}-->':<17}{f'| {round(utnyttelse[i],2)}%'}')
    print('--------UTNYTTELSE----------')
